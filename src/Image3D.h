#ifndef ISOSEGMENTATION_CUDA_IMAGE3D_H
#define ISOSEGMENTATION_CUDA_IMAGE3D_H

#include "XBases.h"

class CImage_3D_Cuda_XGraph_Adaptor : public CImage
{
public:
	typedef CImage BaseClass;
	typedef float ImgType;

	typedef int2 EdgeNodesType;
	typedef int2 NodeEdgesType;
	typedef int2 FacetsType;
	typedef int CellVerticesType;
	typedef float PointCoordsType;
	typedef float BaryCoordsType;

	typedef int NeighborsType;
	typedef int SimilarityType;

	~CImage_3D_Cuda_XGraph_Adaptor();
	void UnifiedMalloc();
	void Synchronize();
...
#endif //ISOSEGMENTATION_CUDA_IMAGE3D_H